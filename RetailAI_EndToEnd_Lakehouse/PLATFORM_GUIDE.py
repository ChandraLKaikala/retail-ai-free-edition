...
# MAGIC The project uses nine schemas: `retail_bronze`, `retail_silver`, `retail_gold`, `retail_ml`, `retail_genai`, `retail_quality`, `retail_monitoring`, `retail_metrics`, and `retail_realtime`.
# MAGIC 
...
# MAGIC - **UTC timezone standardization** (`spark.sql.session.timeZone = UTC`) across all notebooks for consistent timestamp handling
# MAGIC - **Enhanced monitoring schema** with `duration_seconds`, `project_version`, and `environment` in `pipeline_runs`
# MAGIC - **DQ blocking logic**: critical/error severity failures raise exceptions, halting the pipeline
# MAGIC - **DQ failure samples**: `quality_failure_samples` table captures failing records for root-cause analysis
# MAGIC - **Semantic table/column naming**: `fact_inventory_daily_movement`, `baseline_3yr_clv`, `recency_risk`
