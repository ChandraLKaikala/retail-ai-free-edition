...
# DBTITLE 1,Free Edition Design Choices
# MAGIC %md
...
# MAGIC - **UTC timezone standardization:** `spark.sql.session.timeZone = UTC` is set in every notebook for consistent timestamp handling.
# MAGIC - **Enhanced monitoring schema:** `pipeline_runs` includes `duration_seconds`, `project_version`, and `environment` for full audit traceability.
# MAGIC - **DQ blocking logic:** critical and error severity quality failures raise exceptions, halting the pipeline.
# MAGIC - **Semantic table/column naming:** `fact_inventory_daily_movement`, `baseline_3yr_clv`, `recency_risk` for industry-standard accuracy.
