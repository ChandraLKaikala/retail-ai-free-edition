...
# Land a NEW finite batch in the governed Unity Catalog volume.
# This mimics files arriving from an operational source and keeps the demo deterministic/serverless-safe.
...
landed = base.select(
    ...
    F.lit("free_edition_file_demo").alias("source")
)
...
def process_microbatch(batch_df, batch_id):
    ...
    try:
        staged = (batch_df.dropDuplicates(["event_id"])
                  .withColumn("_microbatch_id", F.lit(int(batch_id)))
        ...
        spark.sql(f"""
        MERGE INTO {CAT}.retail_bronze.realtime_order_events t USING _rt_bronze_batch s ON t.event_id=s.event_id
        WHEN MATCHED THEN UPDATE SET * WHEN NOT MATCHED THEN INSERT *
        """)

        silver_batch = (staged.select(
            "order_id","customer_id","store_id","event_time","channel","order_total","discount_amount","tax_amount",
        ...
        keys = silver_batch.select(F.date_trunc("minute","event_time").alias("event_minute"),"channel").distinct()
        keys.createOrReplaceTempView("_rt_keys")
        spark.sql(f"""
        ...
        max_event = staged.agg(F.max("event_time").alias("m")).first()["m"]
...
query = (stream_events.writeStream
...
display(spark.sql(f"""
SELECT stream_name, microbatch_id, input_rows, processing_ms, event_lag_seconds, status, batch_finished_at
FROM {CAT}.retail_monitoring.streaming_metrics
WHERE stream_name='{STREAM_NAME}'
ORDER BY microbatch_id DESC LIMIT 20
"""))
